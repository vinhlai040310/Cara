...
public class ProductImpl implements ProductDao {
    ...
    public List<Product> hot(int limit) {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                prodList.add(mapProduct(rs)); // Gọi phương thức ánh xạ
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Product> news(int limit) {
        ...
        String sql = "SELECT * FROM PRODUCTS ORDER BY created_at DESC LIMIT ?"; // Sản phẩm mới dựa trên thời gian
        try {
            PreparedStatement stmt = con.prepareStatement(sql);
            stmt.setInt(1, limit);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                products.add(mapProduct(rs));  // Gọi phương thức ánh xạ
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
// Phương thức ánh xạ chung cho sản phẩm
